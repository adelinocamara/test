...
document.addEventListener('DOMContentLoaded', function() {
  ...
  btnCalculate.addEventListener('click', function(e) {
    ...
    // Debería forzar el cálculo de resultados
    // Contar respuestas por letra manualmente
    let countA = 0, countB = 0, countC = 0, countD = 0;
    
    console.log('Total de respuestas:', selectedOptions.size);
    ...
    for (const [questionNumber, letter] of selectedOptions.entries()) {
      console.log(`Pregunta ${questionNumber}: ${letter}`);
      switch (letter) {
        case 'a': countA++; break;
        case 'b': countB++; break;
        case 'c': countC++; break;
    ...
    // Forzar actualización de interfaz ANTES de navegar
    updateResults(countA, countB, countC, countD);
    
    // Marcar como calculado
    resultsCalculated = true;
    
    // Primero actualizar el DOM
    setTimeout(function() {
      // Cambiar manualmente las clases activas de los pasos
      document.querySelectorAll('.bs-stepper .step').forEach(function(step, index) {
        if (index === 3) { // El paso de resultados es el 4to (índice 3)
      ...
      // Ocultar todos los contenidos y mostrar sólo el de resultados
      document.querySelectorAll('.bs-stepper-content .content').forEach(function(content) {
        content.classList.remove('active', 'dstepper-block');
        if (content.id === 'results-step') {
          content.classList.add('active', 'dstepper-block');
        }
      });
      
      // Actualizar el índice del stepper
      stepperEl._currentIndex = 3;
      
      // Forzar otra actualización de resultados por si acaso
      setTimeout(function() {
        updateResults(countA, countB, countC, countD);
      }, 100);
  ...
  function updateResults(countA, countB, countC, countD) {
    ...
    if (countA === 0 && countB === 0 && countC === 0 && countD === 0) {
      document.getElementById('result-a-display').textContent = '0';
      document.getElementById('result-b-display').textContent = '0';
      document.getElementById('result-c-display').textContent = '0';
      document.getElementById('result-d-display').textContent = '0';
      
      document.getElementById('sanguine-bar').style.width = '0%';
      document.getElementById('choleric-bar').style.width = '0%';
      document.getElementById('melancholic-bar').style.width = '0%';
      document.getElementById('phlegmatic-bar').style.width = '0%';
      
      document.getElementById('primary-temperament').textContent = '-';
      document.getElementById('secondary-temperament').textContent = '-';
      document.getElementById('temperament-description').innerHTML = '';
    }
    ...
    if (total === 0) {
      console.error('Total de respuestas es 0');
      return;
    }
  ...
  function setEventHandlers() {
    ...
    btnCalculate.addEventListener('click', function(e) {
      ...
      // Navegación directa mediante cambios en el DOM
      setTimeout(function() {
        // Cambiar al paso de resultados
        document.querySelector('[data-target="#weaknesses-step"]').parentElement.classList.remove('active');
        document.querySelector('[data-target="#results-step"]').parentElement.classList.add('active');
        
        document.getElementById('weaknesses-step').classList.remove('active', 'dstepper-block');
        document.getElementById('results-step').classList.add('active', 'dstepper-block');
        
        stepperEl._currentIndex = 3;
        console.log('Navegación completada, índice actual:', stepperEl._currentIndex);
      }, 100);
